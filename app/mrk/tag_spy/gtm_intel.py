...
from .index import GTMRootKeys, evaluations_index, \
    dlvBuiltins_index, macros_index, tags_index, triggers_index
from .gtm_resource import GTMResource, GTMResourceMacro
from .gtm_runtime import GTMRuntime
...
class GTMIntel(object):
    ...
    def process_type(self, property_value):
        ...
        if isinstance(real_type, bool):
            return bool

        # determines if type is int
        if isinstance(real_type, int):
            return int
        ...
        if real_type is list and len(property_value) >= 2:
            ...
            if property_value[0] == 'macro' and isinstance(property_value[1], int):
                return GTMResourceMacro
            ...
            if property_value[0] == 'list' and isinstance(property_value[1], list) and property_value[1][0] == 'map':
                return 'mapping'
            ...
            if property_value[0] == 'list' and isinstance(property_value[1], list) and property_value[1][0] == 'tag':
                return 'sequence'
        ...
        if real_type is list and len(property_value) != 0 and property_value[0] == 'list':
            ...
            elif len(property_value) >= 2 and isinstance(property_value[1], str) and '_' in property_value[1]:
    ...
    def process_macro(self, macro):
        ...
        """
        if self.process_type(macro) is GTMResourceMacro:
            try:
    ...
    def process_predicate(self, predicate_index, detailed=False):
        ...
        try:
            ...
            if not detailed:
                ...
                if isinstance(self.process_type(predicate_evaluated), GTMResourceMacro):
    ...
    def get_by_reference(container: Generator, resource_reference: Union[int, list]) -> Union[dict, list]:
        ...
        elif isinstance(resource_reference, list) and isinstance(resource_reference[0], GTMResourceMacro):
